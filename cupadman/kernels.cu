...
__global__
void slice_gen(const double *model, const double *quat, const double *qvals, const double scale,
               const long long num_pix, const long long size, const long long log_flag, double *view) {
...
__global__
void slice_merge(const double *view, const double *quat, const double *qvals, const long long num_pix,
                 const long long size, double *model, double *mweights) {
	int t = blockIdx.x * blockDim.x + threadIdx.x ;
	if (t >= num_pix)
		return ;
...
void calc_prob_all(const double *lview, const long long ndata, const int *ones, const int *multi,
                   ...
                   const int *c_m, const double init, const double *scales, double *prob_r) {
	long long d, t ;
